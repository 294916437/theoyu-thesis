...
fun MeetingsScreen(
    ...
    LazyColumn(
        ...
        if (upcomingMeetings.isEmpty()) {
            ...
            items(upcomingMeetings, key = { "upcoming-${it.roomId}-${it.roomNo}" }) { MeetingSummaryCard(it, onClick = { onMeetingClick(it) }) }
        }
        item { SectionTitle("最近会议") }
        if (recentMeetings.isEmpty()) {
            item { EmptyState("暂无最近会议") }
        } else {
            items(recentMeetings, key = { "recent-${it.roomId}-${it.roomNo}" }) { MeetingSummaryCard(it, onClick = { onMeetingClick(it) }) }
        }
    }
}
