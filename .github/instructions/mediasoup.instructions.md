---
name: mediasoup
description: 当编写或修改与 Mediasoup 相关的代码时应用此指令，包括：SFU 服务器开发（sfu/ 目录）、Mediasoup Worker/Router/Transport/Producer/Consumer 的创建与配置、mediasoup-client 客户端集成、WebRTC 传输层设置、RTP 能力协商、数据通道（DataChannel）管理，以及 Mediasoup 调试与日志配置。
---
